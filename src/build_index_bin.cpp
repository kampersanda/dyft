...
#endif
#ifdef ALGO_HMS1V
    auto index = std::make_unique<hms1v_index<N>>(base_codes.get(), radius);
#endif
...
#ifdef ALGO_DYFT
    const auto result_path = tfm::format("%s/build_index_bin_%s-%s-%dN-%dR-%dT-%gW-%dK.json",  //
                                         result_dir, algorithm, normalize_filepath(base_path),  //
                                         N, radius, splitthr, in_weight * 100, blocks);
#endif
#ifdef ALGO_HMS1V
    const auto result_path = tfm::format("%s/build_index_bin_hms1v-%s-%dN-%dR.json",  //
                                         result_dir, normalize_filepath(base_path), N, radius);
#endif
